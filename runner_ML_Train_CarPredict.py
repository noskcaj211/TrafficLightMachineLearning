...
def run():
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if traci.trafficlight.getPhase("gneJ5") == 2:
            ...
            print(target_dataCarPredictMeq)
            print("Output Data Meq: ")
            print(outputCarPredictMeq.round())
            print("")
            print("")
            ...
            #input_dataTraffic+=outputCarPredictMeq.eval(session=tf.compat.v1.Session())
            input_dataTraffic+=secondCarMeq

            input_dataTensorTraffic = torch.Tensor(input_dataTraffic)
        ...
        if traci.trafficlight.getPhase("gneJ5") == 0:
            ...
            print(target_dataCarPredictBunt)
            print("Output Data Bunt: ")
            print(outputCarPredictBunt.round())
            print("")
            print("")
...
'''
        ...
        if traci.trafficlight.getPhase("gneJ5") == 0:
            ...
            if traci.inductionloop.getLastStepVehicleNumber("0") or traci.inductionloop.getLastStepVehicleNumber("1") or traci.inductionloop.getLastStepVehicleNumber("2") or traci.inductionloop.getLastStepVehicleNumber("3") or traci.inductionloop.getLastStepVehicleNumber("4") or traci.inductionloop.getLastStepVehicleNumber("5") or traci.inductionloop.getLastStepVehicleNumber("6") or traci.inductionloop.getLastStepVehicleNumber("7"):
                ...
                timeLight += 4
            if timeLight <= 0 and timeLight>=-4:
...
if __name__ == "__main__":
    ...
    learning_rateCarPredictMeq = 0.00001
    ...
    learning_rateCarPredictBunt = 0.00001
    output_sizeCarPredictBunt = 4

    input_sizeTraffic = 51
    hidden_sizeTraffic = 1024
    num_layersTraffic = 4
    dropoutTraffic = .2
    learning_rateTraffic = 0.001
    output_sizeTraffic = 1
    ...
    #modelCarPredictMeq.load_state_dict(torch.load("networkCarPredictMeq_1_3_1.pth"))
    ...
    #modelCarPredictBunt.load_state_dict(torch.load("networkCarPredictBunt_1_3_1.pth"))
    ...
    if cuda:
        modelCarPredict.cuda()
        criterionCarPredict.cuda()
    ...
    #Call model, set optimizer and loss function
    modelTraffic = Model(input_sizeTraffic, hidden_sizeTraffic, num_layersTraffic, dropoutTraffic, output_sizeTraffic)
    optimizerTraffic = torch.optim.Adam(modelTraffic.parameters(), lr = learning_rateTraffic)
    criterionTraffic = nn.CrossEntropyLoss()

    #Run on GPU if available
    if cuda:
        modelTraffic.cuda()
        criterionTraffic.cuda()

    #Total number of parameters
    total_params = sum(p.numel() for p in modelTraffic.parameters())
    print("Total number of parameters in network Traffic: " + str(total_params))
    ...
    hiddenTraffic = modelTraffic.init_states(num_layersTraffic, hidden_sizeTraffic)
    #Run on GPU if available
    if cuda:
        hiddenTraffic = (hiddenTraffic[0].cuda(), hiddenTraffic[1].cuda())
    #Set initial gradients
    modelTraffic.zero_grad()
    #Set initial loss
    lossTraffic = 0
    ...
    while(epochNum<10):
        ...
        torch.save(modelTraffic.state_dict(), "networkTraffic"+"_"+str(runNum)+"_"+str(epochNum)+"_"+str(simNum)+".pth")
        ...
        with open('lossesCarPredictBunt'+"_"+str(runNum)+"_"+str(epochNum)+"_"+str(simNum)+'.txt', 'w') as f:
            for item in lossesCarPredictBunt:
                f.write("%s\n" % item)
        lossesCarPredictBunt = []
        with open('lossesTraffic'+"_"+str(runNum)+"_"+str(epochNum)+"_"+str(simNum)+'.txt', 'w') as f:
            for item in lossesTraffic:
                f.write("%s\n" % item)
        lossesTraffic = []
        simNum+=1
        if simNum == 15:
