...
def run():
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if traci.trafficlight.getPhase("gneJ5") == 0 or traci.trafficlight.getPhase("gneJ5") == 3:
            ...
            i = 0
            while i<len(cars):
                if traci.vehicle.getSpeed(cars[i]) < 1.2:
                    if traci.vehicle.getLaneID(cars[i])=="-gneE5_1":
                        numCars[0]+=1
                    if traci.vehicle.getLaneID(cars[i])=="-gneE5_0":
                        numCars[1]+=1
                    if traci.vehicle.getLaneID(cars[i])=="-gneE4_1":
                        numCars[2]+=1
                    if traci.vehicle.getLaneID(cars[i])=="-gneE4_0":
                        numCars[3]+=1
                i+=1
            i = 0
            while i<4:
                if (int(outputCarPredictBunt[0][0][i].item())-numCars[i])<=10:
                    CarPredictOffNumbers[(int(outputCarPredictBunt[0][0][i].item())-numCars[i])]+=1
                else:
                    CarPredictOffNumbers[10]+=1
                i+=1
            print(timeStep)
            print(CarPredictOffNumbers)
            #print(numCarsOff/timeStep)
            #CarPredictOff.append(numCarsOff/timeStep)
            #print('')

        #bunt green
        if traci.trafficlight.getPhase("gneJ5") == 2 or traci.trafficlight.getPhase("gneJ5") == 1:
            numCars = [0,0,0,0,0,0,0,0]
            i = 0
            while i<len(cars):
                if traci.vehicle.getSpeed(cars[i]) < 1.2:
                    if traci.vehicle.getLaneID(cars[i])=="-gneE7_3":
                        numCars[0]+=1
                    if traci.vehicle.getLaneID(cars[i])=="-gneE7_2":
                        numCars[1]+=1
                    if traci.vehicle.getLaneID(cars[i])=="-gneE7_1":
                        numCars[2]+=1
                    if traci.vehicle.getLaneID(cars[i])=="-gneE7_0":
                        numCars[3]+=1
                    if traci.vehicle.getLaneID(cars[i])=="-gneE6_3":
                        numCars[0]+=1
                    if traci.vehicle.getLaneID(cars[i])=="-gneE6_2":
                        numCars[1]+=1
                    if traci.vehicle.getLaneID(cars[i])=="-gneE6_1":
                        numCars[2]+=1
                    if traci.vehicle.getLaneID(cars[i])=="-gneE6_0":
                        numCars[3]+=1
                i+=1
            i = 0
            while i<8:
                if (int(outputCarPredictMeq[0][0][i].item())-numCars[i])<=10:
                    CarPredictOffNumbers[(int(outputCarPredictMeq[0][0][i].item())-numCars[i])]+=1
                else:
                    CarPredictOffNumbers[10]+=1
                i+=1
            print(timeStep)
...
if __name__ == "__main__":
    ...
    #generate_routefile()
    CarPredictOff = []
    CarPredictOffNumbers = []
    i = 0
    while i < 11:
        CarPredictOffNumbers.append(0)
        i+=1
    ...
    while(simNum<15):
        ...
        traci.start([sumoBinary, "-c", files[simNum-1],
                                "--tripinfo-output", "trash"+"_"+str(simNum)+".xml"])
        '''
        with open('CarPredictOff'+"_"+str(simNum)+'.txt', 'w') as f:
            for item in CarPredictOff:
                f.write("%s\n" % item)
        '''

        run()

        with open('CarPredictOff2'+"_"+str(simNum)+'.txt', 'w') as f:
            f.write("%s\n" % CarPredictOffNumbers)
        CarPredictOffNumbers = []
        i = 0
        while i < 11:
