...
def run():
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if traci.trafficlight.getPhase("gneJ5") == 0:
            i = 0
            while i<len(cars):
                if traci.vehicle.getRoadID(cars[i])=="-gneE4" or traci.vehicle.getRoadID(cars[i])=="-gneE5":
                    if traci.vehicle.getSpeed(cars[i]) < 1:
                        numCars+=1
                i+=1
            predictNumCars = outputCarPredictBunt[0][0][0]+outputCarPredictBunt[0][0][1]+outputCarPredictBunt[0][0][2]+outputCarPredictBunt[0][0][3]
            numCarsOff += abs(numCars-torch.sum(outputCarPredictBunt))
        ...
        if traci.trafficlight.getPhase("gneJ5") == 2:
            i = 0
            while i<len(cars):
                if traci.vehicle.getRoadID(cars[i])=="-gneE6" or traci.vehicle.getRoadID(cars[i])=="-gneE7":
                    if traci.vehicle.getSpeed(cars[i]) < 1:
                        numCars+=1
                i+=1
            predictNumCars = outputCarPredictMeq[0][0][0]+outputCarPredictMeq[0][0][1]+outputCarPredictMeq[0][0][2]+outputCarPredictMeq[0][0][3]+outputCarPredictMeq[0][0][4]+outputCarPredictMeq[0][0][5]+outputCarPredictMeq[0][0][6]+outputCarPredictMeq[0][0][7]
            numCarsOff += abs(numCars-torch.sum(outputCarPredictMeq))
            print(numCarsOff/timeStep)
            CarPredictOff.append(numCarsOff/timeStep)
            print('')
        #print(carsData)
